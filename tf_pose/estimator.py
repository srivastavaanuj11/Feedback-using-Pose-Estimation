...
class PoseEstimator:
    ...
    def estimate_paf(peaks, heat_mat, paf_mat):
        ...
        humans = []
        stlist = list()
        stlist.append("Angle in degrees between points 1 2 3 is 0 "+" r")
        stlist.append("Angle in degrees between points 1 5 6 is 0 "+" r")
        ...
        for human_id in range(pafprocess.get_num_humans()):
            ...
            for part_idx in range(18):
                ...
                print(human.body_parts[part_idx],float(pafprocess.get_part_x(c_idx)),float(pafprocess.get_part_y(c_idx)))
                ...
                try:
                    if((1 in parts1) and (2 in parts1) and (3 in parts1)):
                        a =1
                        b = 2
                        c = 3
                        x1 = float(pafprocess.get_part_x(a))
                        y1 = float(pafprocess.get_part_y(a))
                        x2 = float(pafprocess.get_part_x(b))
                        y2 = float(pafprocess.get_part_y(b))
                        x3 = float(pafprocess.get_part_x(c)) 
                        y3 = float(pafprocess.get_part_y(c))
                        slope1 = slope(x1,y1,x2,y2)
                        slope2 = slope(x2,y2,x3,y3)
                        ang1 = angle(slope1, slope2)
                        
                        
                        print('Angle in degrees = ', ang1)
                ...
                try:
                    if((1 in parts1) and (5 in parts1) and (6 in parts1)):
                        a =1
                        b = 5
                        c = 6
                        x1 = float(pafprocess.get_part_x(a))
                        ...
                        slope1 = slope(x1,y1,x2,y2)
                        slope2 = slope(x2,y2,x3,y3)
                        ang2 = angle(slope1, slope2)
                        
                           
                        print('Angle in degrees = ', ang2)
                ...
                stlist = list()
                if(ang1>-11):
                    st1 = "Angle in degrees between points 1 2 3 is "+str(ang1)+" w"
                else:
                    st1 = "Angle in degrees between points 1 2 3 is "+str(ang1)+" r"
                if(ang2<11):
                    st2 = "Angle in degrees between points 1 5 6 is "+str(ang2)+" w"
                else:
                    st2 = "Angle in degrees between points 1 5 6 is "+str(ang2)+" r"
...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_humans(npimg, humans,stlist, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                centers[i] = center
                cv2.circle(npimg, center, 3, (255,255,255), thickness=3, lineType=8, shift=0)#common.CocoColors[i]
                cv2.putText(npimg,str(i),center, cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
                n = 30
                ...
                for k in stlist:
                    n = n+20
                    # if(k[-1] == "w" and k[-3]=="6"):
                    #     body_part = human.body_parts[5]
                    #     center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))

                    #     cv2.circle(npimg, center, 3, (255,0,0), thickness=3, lineType=8, shift=0)#common.CocoColors[i]
                    # if(k[-1] == "w" and k[-3]=="3"):
                    ...
                    cv2.putText(npimg,
                        k ,
                        (10, n),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 2)
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        t = time.time()
        stlist = list()
        humans,stlist = PoseEstimator.estimate_paf(peaks, self.heatMat, self.pafMat)
